...
description: "在 Caelestia dotfiles 上接入 HyprGlass 插件：两套可切换的窗口玻璃预设（微磨砂 / 纯清透）、侧边栏图层玻璃与遮罩阈值、窗口透明度，以及色散、紫色描边、QML 重载等常见问题的处理方法。"
category: "计算机科学"
tags:
  - "Hyprland"
  - "Caelestia"
...
这篇笔记记录在 Caelestia（Hyprland + Quickshell 桌面方案）上把窗口和侧边栏都调成液态玻璃的完整过程，环境为 Hyprland 0.56.2、caelestia-shell 2.4.0，插件用 hyprpm 安装。所有改动都在用户配置里，不动 dotfiles 仓库，升级不会冲突。
...
在 `~/.config/caelestia/hypr-user.lua` 里定义两个预设：`liquid` 是带一点磨砂与色调映射的日常版，`crystal` 完全不要模糊和磨砂、只保留折射与高光的清透版。
...
if hl.plugin.hyprglass then
    ...
    -- 纯清透版：无模糊、无 tint、不做色调映射，只有折射与边缘高光
    hg.preset("crystal", {
        inherits = "liquid",
        blur_strength = 0,
...
玻璃是画在窗口下面的，窗口自身不透明的话等于白做。Caelestia 的默认窗口不透明度在 `~/.config/hypr/variables.lua`：

```lua
windowOpacity = 0.88,   -- 原为 0.95，玻璃几乎透不出来
```

注意 `rules.lua` 里 `foot`、图片查看器、剪辑软件等被打了 `opaque` 标签，它们不会参与透明与玻璃；想让终端也有玻璃质感，把 `foot` 从 opaque 列表里删掉即可。
...
侧边栏专用预设继承 `crystal`，因为面板面积大，折射和边缘参数比其他窗口更收敛：
...
这里最大的坑是 `mask_threshold`。图层玻璃只画在「可见内容 alpha 超过阈值」的地方，而 Caelestia 面板填充的 alpha 是两层相乘的：`appearance.transparency.base`（当时是 0.1）乘以面板 Item 的 `opacity`（同样取 `surfaceColour.a`，也是 0.1），实际只有约 0.016。阈值设成 0.05 时，玻璃只会出现在文字和图标背后，整块面板没有玻璃；改成 0.008 才会覆盖整个面板（同时略微盖到面板外阴影，属于可接受范围）。如果以后调高 `shell.json` 里的 `transparency.base`，阈值也要跟着调。

另外为了让面板背景真正透明，把 `modules/sidebar/Content.qml` 里通知区那层 `color: Colours.tPalette.m3surfaceContainerLow` 改成了 `color: "transparent"`，否则玻璃上还盖着一层 10% 的面板底色。
...
**聚焦窗口出现紫色描边**：这不是玻璃，是 Hyprland 的窗口边框 `general:col:active_border` 取了动态配色里的 `primary`（比如 `c2c1ff`）。想彻底关掉就在 `hypr-vars.lua` 里写 `windowBorderSize = 0`；它在默认配置之前生效，reload 也不会被覆盖回去。裸写在 `hypr-user.lua` 里的 `general = { border_size = 0 }` 实测 reload 后不生效。
...
最终状态：窗口默认 `crystal`（无模糊、无磨砂的清透玻璃，`liquid` 微磨砂版随时可切回），不透明度 0.88，无窗口边框；侧边栏无模糊、无染色、面板底色全透明，`mask_threshold = 0.008` 让玻璃铺满整块面板。所有原始配置均有带时间戳的 `.bak` 备份，可随时回滚。
